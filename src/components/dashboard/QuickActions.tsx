      ...
      <div className="flex flex-row gap-3 overflow-x-auto pb-2 items-stretch">
        {quickActionsList.map((action) => (
          <Button 
            ...
            variant="outline" 
            size="sm" 
            className="h-auto px-3 py-2 flex flex-col items-center justify-center text-center w-[100px] shrink-0" 
            asChild={!action.isLoading && !!action.href && action.href !== '#'}
            disabled={action.isLoading || !action.href || action.href === '#'}
            onClick={action.href === '#' ? () => alert('AI Assistant coming soon!') : undefined}
          >
            {action.isLoading ? (
              <div className="flex flex-col items-center justify-center">
                <Loader2 className="h-5 w-5 mb-1 animate-spin text-primary" />
                <span className="text-xs font-medium leading-tight block whitespace-nowrap overflow-hidden text-ellipsis w-full">{action.label}</span>
              </div>
            ) : (
              <Link href={action.href || "#"} target={action.isExternal ? "_blank" : "_self"} rel={action.isExternal ? "noopener noreferrer" : undefined} passHref>
                <action.icon className="h-5 w-5 mb-1 text-primary" />
                <span className="text-xs font-medium leading-tight block whitespace-nowrap overflow-hidden text-ellipsis w-full">{action.label}</span>
              </Link>
            )}
          </Button>
